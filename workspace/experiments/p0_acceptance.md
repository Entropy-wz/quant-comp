...
Evidence collected on branch `p0-foundation` (HEAD `d13a68b`+). Unit suite re-run for this checklist.
...
- [x] Official runner succeeds for `strategy_v0` — **smoke gate OK** (`runner_gate_v0_smoke.json`: `status=ok`, 300 predict calls, 0 timeouts). **Full gate not run on this host:** `run_runner_gate_v0.py --mode full` fails with `MemoryError` loading full `test_partition_000.parquet` (~1M rows × 323 features); see `runner_gate_stdout.txt`. Use smoke gate for P0 CI; run full gate on a RAM-capable host.
